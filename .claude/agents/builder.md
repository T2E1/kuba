...
| `settings.json` → `hooks` | O que roda em torno de cada prompt e cada ferramenta |
| `hooks/validate.py` | Valida as quatro camadas do `.claude/` a cada prompt |
...
| Vale a cada prompt, e custa menos de 0,2s | `UserPromptSubmit` em `settings.json` |
