# Receitas

Receitas completas e funcionais — telas inteiras, não elementos isolados. Cada
uma parte de um markup que você pode colar numa página com as duas tags da
[Instalação](/pt-br/learn/installation), e evolui até a versão que você
publicaria.

As receitas assumem que você leu [Eventos e Echo](/pt-br/learn/events-and-echo);
boa parte do que as deixa curtas é fiação por arcos.

## Receitas

- **[Busca enquanto digita](/pt-br/cookbook/search-as-you-type)** — um input
  disparando uma requisição, resultados renderizados de um template, com estados
  de erro e vazio. Três elementos, nenhum listener.
- **[CRUD de usuários](/pt-br/cookbook/user-crud)** — adicionar, listar e
  excluir registros. Quatro arcos fechando um ciclo: formulário → dataset →
  lista → dataset.
- **[Navegação declarativa](/pt-br/cookbook/declarative-navigation)** — um botão
  que navega sem conhecer o destino, e um único redirect servindo uma lista
  inteira.

## Uma nota sobre nomes

O barramento do Echo é compartilhado pela página, e um arco casa sua origem por
`id`, `name` ou nome de tag. Duas funcionalidades que nomeiem um elemento como
`users` vão se cruzar — o arco dispara para qualquer uma delas.

As receitas aqui prefixam seus nomes (`crud-users`, `crud-form`) porque este site
renderiza vários exemplos ao vivo por página. Numa aplicação, dê aos nomes o
escopo da funcionalidade pelo mesmo motivo.
