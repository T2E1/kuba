...
| Comando | Faz |
|---|---|
| `bun run dev` | Serve esta documentação em `localhost:3000`. |
| `bun run test` | Roda a suíte de testes num Chromium real, via Vitest. |
| `bun run check` | Lint e formatação, via Biome. |
| `bun run release` | Constrói o `dist/` com o Vite. |
...
Antes de escrever qualquer coisa, confira [Nomenclatura](/pt-br/foundations/naming) —
...
tokens à sua marca — veja [Temas](/pt-br/build-ui/theming) — e ajuste os
...
garantias vêm dos [princípios](/pt-br/foundations/principles), não de uma API
