...
- **Pode conter**: qualquer coisa. O shadow root é um único `<slot>` sem nome,
  então todo filho renderiza na ordem do código como um item flex.
...
`direction` é o único atributo com um conjunto fechado de valores. `align` e
`justify` passam direto para `align-items` e `justify-content` ao pé da letra,
então qualquer valor CSS válido funciona — e um inválido silenciosamente não faz
nada.

| Atributo | Age ao longo de | Valores comuns |
|---|---|---|
| `justify` | a direção do stack (eixo principal) | `flex-start`, `center`, `space-between`, `flex-end` |
| `align` | transversal a ela (eixo cruzado) | `start`, `center`, `stretch`, `baseline` |
...
| `lg` para cima | 40px+ | Separação em nível de seção, onde o `<kb-inset>` pode servir melhor. |
...
| `align` | `align-items` do CSS | `start` | Alinhamento no eixo cruzado. |
| `justify` | `justify-content` do CSS | `flex-start` | Alinhamento no eixo principal. |
| `spacing` | degrau de token | `xs` | Espaçamento, resolvido contra `--spacing_inset-{valor}`. |
...
- **O host é declarado presentacional**, então a pilha em si não acrescenta nó
  nenhum à árvore de acessibilidade. A semântica de grupo tem que vir do que
