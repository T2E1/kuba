...
Recipes assume you've read [Events and Echo](/learn/events-and-echo); most of
what makes them short is arc wiring.

## Recipes

- **[Search as you type](/cookbook/search-as-you-type)** — an input driving a
  request, results rendered from a template, with error and empty states. Three
  elements, no listeners.
- **[User CRUD](/cookbook/user-crud)** — add, list and delete records. Four
  arcs that close a cycle: form → dataset → list → dataset.
- **[Declarative navigation](/cookbook/declarative-navigation)** — a button
