...
| Comando | Hace |
|---|---|
| `bun run dev` | Sirve esta documentación en `localhost:3000`. |
| `bun run test` | Ejecuta la suite de pruebas en un Chromium real, vía Vitest. |
| `bun run check` | Lint y formato, vía Biome. |
| `bun run release` | Construye `dist/` con Vite. |
...
Antes de escribir nada, revisa [Nomenclatura](/es/foundations/naming) — el
...
design tokens a tu marca — mira [Temas](/es/build-ui/theming) — y ajusta los
...
vienen de los [principios](/es/foundations/principles), no de una API que deba
