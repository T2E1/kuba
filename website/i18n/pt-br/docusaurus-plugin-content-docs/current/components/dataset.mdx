...
!> Com o `upsert` não definido, cada `push` gera seu próprio uuid, então nenhum
registro compartilha chave com outro — nada nunca é mesclado, e cada push
silenciosamente adiciona um registro novo em vez de atualizar um existente.
Defina sempre que os registros tiverem qualquer identidade.
...
| Faça | Não faça |
|---|---|
| Definir `upsert` com um campo identificador de verdade | Deixá-lo indefinido e ver cada push adicionar silenciosamente um duplicado em vez de atualizar |
