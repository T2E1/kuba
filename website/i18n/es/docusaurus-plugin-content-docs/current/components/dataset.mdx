...
!> Con `upsert` sin definir, cada `push` genera su propio uuid, así que ningún
registro comparte clave con otro — nada se fusiona nunca, y cada push agrega
silenciosamente un registro nuevo en lugar de actualizar uno existente.
Defínelo siempre que los registros tengan cualquier identidad.
...
| Haz | No hagas |
|---|---|
| Establecer `upsert` con un campo identificador real | Dejarlo sin definir y ver cómo cada push agrega silenciosamente un duplicado en vez de actualizar |
