...
document.querySelector('kb-dataset').addEventListener('found', (event) => {
  showDetail(event.detail) // un registro, o null
})
```
...
La búsqueda se ejecuta cuando **`value` cambia**, y solo entonces — a
diferencia de `<kb-filter>`, cambiar `key` solo no la dispara aquí. Un registro
empujado al padre después tampoco la vuelve a disparar; reasigna `value` para
forzar una nueva pasada. Asignar el *mismo* valor dos veces tampoco la vuelve a
disparar necesariamente, ya que el setter se ejecuta al cambiar el atributo.

Dos escrituras en `value` hechas dentro de aproximadamente 100ms una de la otra
colapsan en una sola búsqueda y un solo evento `found`, en vez de una por
escritura.
...
|---|---|---|
| `found` | **el elemento padre** | el primer registro coincidente, o `null` |

!> **Ninguna coincidencia despacha `null`, no un objeto vacío.** Un
...
| Manejar el caso `null` de una búsqueda sin resultado | Suponer que siempre vuelve un registro |
