...
- **Puede ser hijo de**: cualquier componente que exponga un slot `helper` —
  `<kb-input>` o `<kb-textarea>`. Colocado en cualquier otro sitio sigue
  renderizando, pero el `slot="helper"` que se pone a sí mismo no coincide con
  nada.
...
corrige el error, repítelo en el mensaje de validez.
