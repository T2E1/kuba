...
- **Puede contener**: cualquier cosa — el shadow root es un único `<slot>` sin
  ...
  lugar de escaparse de ellas. Uno o más hijos `<kb-on>` añaden arcos más allá
  del único atributo `on`.
...
Un valor fuera del conjunto se ignora: el cambio de atributo nunca llega a la
propiedad, que conserva su último valor válido (`all` hasta que definas uno).
Lo mismo vale para `direction`.
...
| `width` | `hug` \| `fill` \| longitud | `auto` | Ancho del host, normalizado a un conjunto cerrado de formas de tamaño. |
| `height` | `hug` \| `fill` \| longitud | `auto` | Alto del host, normalizado de la misma manera. |
| `hidden` | `boolean` | `false` | Elimina el elemento y su contenido del layout y del árbol de accesibilidad. |
| `alt` | `string` | `''` | Nombre accesible. Rara vez relevante — el host es de presentación. |
| `on` | arc string | — | Cableado del Echo, `source/event:type/sink` — deja que otro elemento controle un atributo de este inset. |
