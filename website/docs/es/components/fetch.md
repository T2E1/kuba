...
| `failed` | la petición o su parseo falló | el error |

!> **`failed` lleva el error en su `detail`.** Léelo directamente en el
listener, o haz `await` sobre la llamada al método, que resuelve con
`{ data, error }`.
...
| Leer el `detail` de `failed` para el error, o conectarlo a una reacción como `clear` | Suponer que `detail` es el cuerpo de la respuesta — en `failed` es el error |
