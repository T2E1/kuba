...
| [`@t2e1/kuba/directive`](/build-elements/directive) | Decorators de ciclo de vida: `define`, `connected`, `disconnected`, `adopted`, `attributeChanged`, `formAssociated`, `formDisabled`, `formReset`, `formStateRestore`, `execute`. |
| [`@t2e1/kuba/dom`](/build-elements/dom) | Renderizado: `paint`, `repaint`, `retouch`, `html`, `css`, y los símbolos del ciclo de paint. |
| [`@t2e1/kuba/mixin`](/build-elements/mixin) | Mixins de atributo: `Headless`, `Height`, `Hidden`, `Identity`, `Template`, `Value`, `Width`. |
| [`@t2e1/kuba/echo`](/build-elements/echo) | El mixin host `Echo` y el decorator `dispatchEvent`. |
| [`@t2e1/kuba/event`](/build-elements/event) | El proxy de listener `on` y sus filtros: `value`, `files`, `formData`, `dataset`, `detail`, `prevent`, `stop`, `customEvent`. |
| `@t2e1/kuba/middleware` | `before`, `after`, `around` — mira [Decorators](/build-elements/decorators). |
| [`@t2e1/kuba/spark`](/build-elements/spark) | El registro de operadores que usan los filtros de arco de Echo. |
| [`@t2e1/kuba/http`](/build-elements/http) | Un wrapper de `fetch` que devuelve un objeto de resultado en lugar de lanzar. |
| `@t2e1/kuba/result` | El tipo de resultado que usa `http`. |
| [`@t2e1/kuba/router`](/build-elements/router) | Registro de rutas, `params`, `args`, `urlFor` y el evento `pushstate`. |
...
cambio. Las páginas de **[Design tokens](/foundations/tokens/)** renderizan
