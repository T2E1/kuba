...
| `<kb-input>`, `<kb-textarea>` | `changed` | o novo valor |
...
| `<kb-fetch>` | `failed` | `null` — veja [Fetch](/components/fetch) |
...
- **[Decorators](/build-elements/decorators)** — `@on` para escutar dentro do seu
  próprio componente, e os decorators de middleware.
- **[Receitas › Busca enquanto digita](/build-ui/patterns/search-as-you-type)** — o
