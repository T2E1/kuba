...
document.querySelector('kb-dataset').addEventListener('found', (event) => {
  showDetail(event.detail) // um registro, ou null
})
```
...
A busca roda quando **`value` muda**, e só então — diferente do `<kb-filter>`,
mudar `key` sozinho não a redispara aqui. Um registro empurrado ao pai depois
disso também não a redispara; reatribua o `value` para forçar uma nova passada.
Atribuir o *mesmo* valor duas vezes também não necessariamente redispara, já que
o setter roda na mudança de atributo.

Duas escritas em `value` feitas dentro de aproximadamente 100ms uma da outra
colapsam numa única busca e num único evento `found`, em vez de uma por
escrita.
...
|---|---|---|
| `found` | **o elemento pai** | o primeiro registro correspondente, ou `null` |

!> **Nenhuma correspondência dispara `null`, não um objeto vazio.** Um
...
| Tratar o caso `null` de uma busca sem resultado | Supor que um registro sempre volta |
