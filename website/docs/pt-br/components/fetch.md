...
| `failed` | a requisição ou a conversão falhou | o erro |

!> **O `failed` carrega o erro no `detail`.** Leia direto no listener, ou dê
`await` na chamada do método, que resolve com `{ data, error }`.
...
| Ler o `detail` do `failed` para o erro, ou ligá-lo a uma reação como o `clear` | Supor que o `detail` é o corpo da resposta — no `failed` ele é o erro |
