...
- **Pode conter**: qualquer coisa — o shadow root é um único `<slot>` sem nome.
  ...
  que a caixa é cortada pelos cantos arredondados em vez de escapar deles. Um
  ou mais filhos `<kb-on>` adicionam arcos além do único atributo `on`.
...
Um valor fora do conjunto é ignorado: a mudança de atributo nunca chega à
propriedade, que mantém o último valor válido (`all` até você definir um). O
mesmo vale para `direction`.
...
| `width` | `hug` \| `fill` \| comprimento | `auto` | Largura do host, normalizada para um conjunto fechado de formas de tamanho. |
| `height` | `hug` \| `fill` \| comprimento | `auto` | Altura do host, normalizada da mesma maneira. |
| `hidden` | `boolean` | `false` | Remove o elemento e seu conteúdo do layout e da árvore de acessibilidade. |
| `alt` | `string` | `''` | Nome acessível. Raramente relevante — o host é de apresentação. |
| `on` | arc string | — | Fiação do Echo, `source/event:type/sink` — deixa outro elemento controlar um atributo deste inset. |

Este elemento não dispara eventos; `on` apenas recebe.
