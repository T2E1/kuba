...
`<kb-input>` e `<kb-textarea>` — mais um `<kb-button type="submit">`. Por serem
associados a formulário, eles se registram no `<form>` interno e aparecem nos
dados submetidos pelo `name`.
