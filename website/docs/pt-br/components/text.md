...
- **Pode conter**: texto e markup inline — o shadow root é um único `<slot>` sem
  ...
  definidos aqui. Um ou mais filhos `<kb-on>` adicionam arcos além do único
  atributo `on`.
...
`color` aceita qualquer sufixo de `--color-*` como um conjunto fechado de
palavras-chave. Use as famílias semânticas para significado, e a rampa neutra
`master-*` para hierarquia.
...
Uma palavra-chave fora do conjunto é ignorada: a mudança de atributo não chega
à propriedade, que mantém o último valor válido (`master-dark` até você definir
um) — confira o nome do token se uma cor "não aplica". O mesmo vale para `size`,
`weight`, `family`, `line-height` e `align`.
...
| `on` | string de arco | — | Fiação do Echo, `source/event:type/sink` — deixa outro elemento dirigir um atributo deste texto. |
