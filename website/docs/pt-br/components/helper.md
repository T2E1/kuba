...
- **Pode ser filho de**: qualquer componente que exponha um slot `helper` —
  `<kb-input>` ou `<kb-textarea>`. Colocado em qualquer outro lugar ele ainda
  renderiza, mas o `slot="helper"` que define em si mesmo não casa com nada.
...
enquanto corrige o erro, repita-a na mensagem de validade.
