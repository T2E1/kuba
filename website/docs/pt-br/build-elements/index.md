...
| [`@t2e1/kuba/dom`](/build-elements/dom) | Renderização: `paint`, `repaint`, `retouch`, `html`, `css`, e os símbolos do ciclo de paint. |
| [`@t2e1/kuba/mixin`](/build-elements/mixin) | Mixins de atributo: `Headless`, `Height`, `Hidden`, `Identity`, `Template`, `Value`, `Width`. |
| [`@t2e1/kuba/echo`](/build-elements/echo) | O mixin host `Echo` e o decorator `dispatchEvent`. |
| [`@t2e1/kuba/event`](/build-elements/event) | O proxy de listener `on` e seus filtros: `value`, `files`, `formData`, `dataset`, `detail`, `prevent`, `stop`, `customEvent`. |
| `@t2e1/kuba/middleware` | `before`, `after`, `around` — veja [Decorators](/build-elements/decorators). |
| [`@t2e1/kuba/spark`](/build-elements/spark) | O registro de operadores usado pelos filtros de arco do Echo. |
| [`@t2e1/kuba/http`](/build-elements/http) | Um wrapper de `fetch` que devolve um objeto de resultado em vez de lançar erro. |
| `@t2e1/kuba/result` | O tipo de resultado usado pelo `http`. |
| [`@t2e1/kuba/router`](/build-elements/router) | Registro de rotas, `params`, `args`, `urlFor` e o evento `pushstate`. |
...
**[Design tokens](/foundations/tokens/)** renderizam cada escala ao vivo,
