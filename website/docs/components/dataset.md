...
!> With `upsert` unset, every `push` generates its own uuid, so no two records
ever share a key — nothing is ever merged, and every push silently adds a new
record instead of updating an existing one. Set it whenever records have any
identity at all.
...
| Do | Don't |
|---|---|
| Set `upsert` to a real identifying field | Leave it unset and watch every push silently add a duplicate instead of updating |
