...
- **Can contain**: anything — the shadow root is a single unnamed `<slot>`.
  ...
  the box is clipped by the rounded corners rather than escaping them. One or
  more `<kb-on>` children add arcs beyond the single `on` attribute.
...
A value outside the set is ignored: the attribute change never reaches the
property, which keeps its last valid value (`all` until you set one). The same
holds for `direction`.
...
| `width` | `hug` \| `fill` \| length | `auto` | Host width, normalized to a closed set of size shapes. |
| `height` | `hug` \| `fill` \| length | `auto` | Host height, normalized the same way. |
| `hidden` | `boolean` | `false` | Removes the element and its content from layout and the accessibility tree. |
| `alt` | `string` | `''` | Accessible name. Rarely meaningful — the host is presentational. |
| `on` | arc string | — | Echo wiring, `source/event:type/sink` — lets another element drive an attribute of this inset. |
