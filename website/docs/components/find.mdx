...
document.querySelector('kb-dataset').addEventListener('found', (event) => {
  showDetail(event.detail) // one record, or null
})
```
...
The search runs when **`value` changes**, and only then — unlike `<kb-filter>`,
changing `key` alone doesn't re-trigger it here. A record pushed to the parent
afterwards doesn't re-trigger it either; re-assign `value` to force a new pass.
Assigning the *same* value twice doesn't necessarily re-fire either, since the
setter runs on attribute change.

Two writes to `value` made within roughly 100ms of each other collapse into a
single search and a single `found` event, instead of one per write.
...
|---|---|---|
| `found` | **the parent element** | the first matching record, or `null` |

!> **No match dispatches `null`, not an empty object.** A consumer that
...
| Handle the `null` case for a miss | Assume a record always comes back |
