...
- **Filtering something that isn't a parent collection.** It reads
  `parentElement.value` with no guard — a parent without a `value` array, or no
  parent at all, throws instead of doing nothing.
...
Filtering runs when **`key` or `value` changes** — either one triggers a new
pass. Two consequences:

- **Setting either attribute is what fires it**, so a filter with static
  attributes evaluates once, at upgrade, against whatever the parent held at
  that moment. Setting both at parse time counts as two triggers, but they're
  coalesced (see below) into a single pass.
- **A later `push` on the parent does not re-filter.** The dataset dispatches
  ...
  `key` or `value` to force a new pass, or wire the consumer to the parent's
  `changed` instead.

Two writes to `key` and/or `value` made within roughly 100ms of each other
collapse into a single pass and a single `filtered` event, instead of one per
write — so a `<kb-filter key="…" value="…">` declared with both attributes at
parse time still only publishes once.

## Attributes

| Attribute | Type | Default | Description |
|---|---|---|---|
| `key` | `string` | — | Record field to compare. Assigning it triggers a new pass, same as `value`. |
