...
| `failed` | the request or its parsing failed | the error |

!> **`failed` carries the error in its `detail`.** Read it directly on the
listener, or `await` the method call, which resolves with `{ data, error }`.
...
| Read `failed`'s `detail` for the error, or wire it to a reaction like `clear` | Assume `detail` is the response body — on `failed` it's the error |
