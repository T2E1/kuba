...
`width="fill"` call to action. `circular` is for perfectly round elements like
avatars.
...
`hairline` is the default for anything that borders a surface — inputs and
outlined cards. `thin` is the button's border, so an outlined
